...
from .base import normalize_tags, json_response, error_response
...
VERBOSE entries, full context, more is better. Never lose: exact quotes, numbers, names, dates, examples given, caveats stated, and how ideas developed. Summarizing loses information.""",
    inputSchema={
        ...
        "properties": {
            ...
            "category": {"type": "string", "enum": ["reference", "pattern", "table", "command", "issue", "troubleshooting", "project", "decision", "research", "log", "todo", "seed", "transcript", "other"]},
...
async def execute(con, args: dict) -> List[TextContent]:
    ...
    # Block protected entry types - use dedicated tools instead
    if entry_id.startswith("todo-"):
        return error_response("protected_entry",
            f"Cannot upsert '{entry_id}'. Use todo_add/todo_complete tools instead.")
    if entry_id.startswith("accumulator-"):
        return error_response("protected_entry",
            f"Cannot upsert '{entry_id}'. Use append_accumulator tool instead.")
