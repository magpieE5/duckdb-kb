...
Provides 14 tools for Claude Code to interact with knowledge base:
...
- System: initialize_database, git_commit_and_get_sha, get_kb_session_status
"""
...
Returns: {"success": true, "sha": "abc123..."} or error""",
        ...
        Tool(
            ...
            description="""Get KB initialization status, parse KB.md for focus areas and commitments.

Returns structured JSON with:
- database.action: "init_db_fresh", "init_db_restore", or "check_empty"
- kb_md.action: "setup_kb_md" if template, "ready" if populated
- status.focus_areas: Top 5 parsed from KB.md Current Focus
...
async def tool_get_kb_session_status(arguments: dict) -> list[TextContent]:
    """Get KB initialization status, parse KB.md, check deadlines"""
    import re
    from datetime import datetime, timedelta

    project_dir = Path(__file__).parent
    db_path = project_dir / "kb.duckdb"
    kb_md_path = project_dir / ".claude" / "KB.md"
    ...
    if not kb_md_path.exists():
        result["kb_md"] = {
            ...
            "message": "KB.md not found - needs creation"
        }
    else:
        ...
        result["kb_md"] = {
            ...
            "message": "KB.md needs initial setup" if is_template else "KB.md ready"
        }
